...
export const BarraNavegacion: React.FC<BarraNavegacionProps> = ({ 
  ...
  return (
    <nav className="px-6 flex justify-center gap-4 flex-wrap mt-8 relative z-10">
      {categorias.map(c => (
        <motion.button 
          key={c.nombre} 
          ...
          whileTap={{ scale: 0.95 }}
          className={`flex items-center gap-3 px-6 py-3 rounded-2xl transition-all duration-300 font-semibold tracking-wide border-2 ${
            filtro === c.nombre 
              ? 'neon-button-active text-white bg-black-50' 
              : 'neon-button-inactive text-white-80 bg-black-40 hover:shadow-[0_0_20px_rgba(0,255,247,0.4)]'
          }`}
        >
          {c.icono}
